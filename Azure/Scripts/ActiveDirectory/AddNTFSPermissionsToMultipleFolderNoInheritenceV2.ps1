# Works but disables inheritance on all folders. Need to add a flag to the CSV to disable inheritance on a folder by folder basis.
$csvPath = "d:\scripts\Permis.csv"
...
foreach ($entry in $permissionsList) {
    ...
    try {
        ...
        if ($disableInheritance) {
            $acl.SetAccessRuleProtection($true, $false)  # Disables inheritance and removes inherited permissions
            Write-Output "Inheritance Disabled for $folderPath"
        } else {
