...
:: Define working folders (all under \images)
...
rem if not exist "%IMAGES%" mkdir "%IMAGES%"
rem if not exist "%MOUNT%" mkdir "%MOUNT%"
rem if not exist "%WINREMOUNT%" mkdir "%WINREMOUNT%"
...
:: Set source WIM path
set "WIMFILE=%BASEDIR%source\active\install.wim"
