...
@echo *********************************************************************
@echo Finding the device model name
for /f "tokens=2*" %%A in ('reg query "HKEY_LOCAL_MACHINE\HARDWARE\DESCRIPTION\System\BIOS" /v SystemProductName') do set "model=%%B"
echo Device Model: %model%
pause
@echo off
echo ==========================================
echo Getting model and prompting for PC name...
echo ==========================================

powershell.exe -ExecutionPolicy Bypass -NoProfile -Command ^
    "$model = (Get-CimInstance -ClassName Win32_ComputerSystem).Model; ^
     Write-Host 'System Model:' $model; ^
     $newName = Read-Host 'Enter a name for this computer'; ^
     Set-Content -Path 'C:\Windows\Setup\Scripts\ComputerName.txt' -Value $newName"
pause
@echo off
setlocal enabledelayedexpansion
COLOR 1F

:: Set base customer folder path
set "basePath=/customers"

:: Build menu dynamically
:MainMenu
cls
echo ============================================
echo         Pick Customer Folder
echo ============================================
echo.
...
:END
@echo *********************************************************************
:: Perform Unattend.xml File Copy
@echo *********************************************************************
@echo == Perform Unattend.xml File Copy
@echo *********************************************************************
echo Copying unattend files from %basePath%\!selectedFolder!\Unattend\ to W:\Windows\Panther...
xcopy /C /Y "\customers\!selectedFolder!\Unattend\unattend.xml" "W:\Windows\Panther\"
echo.
echo Done.
pause
:: Perform Scripts File Copy
@echo *********************************************************************
@echo == Perform Scripts File Copy
@echo *********************************************************************
echo Copying unattend files from %basePath%\!selectedFolder!\ to W:\Windows\Setup\Scripts...
xcopy /E /I /H /C /Y "\customers\!selectedFolder!\Scripts\*" "W:\Windows\Setup\Scripts\"
echo.
echo Done.
pause
...
if exist "\Drivers\%model%" (
    echo Adding drivers from \Drivers\%model%...
    dism /image:W:\ /add-driver /driver:"\Drivers\%model%" /recurse /forceunsigned
) else (
    echo No driver folder found for %model%. Skipping...
)
pause